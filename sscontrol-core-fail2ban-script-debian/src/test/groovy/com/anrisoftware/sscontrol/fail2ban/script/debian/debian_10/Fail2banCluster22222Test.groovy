 ...
 */
package com.anrisoftware.sscontrol.fail2ban.script.debian.debian_10
...
''',
            ...
            expected: { Map args ->
                ...
                assertStringResource Fail2banServerTest, readRemoteFile('/etc/fail2ban/filter.d/sshd.conf', 'robobee-test.test'), "${args.test.name}_sshd_conf_expected.txt"
                (1..2).each {
                    ...
                    assertStringResource Fail2banServerTest, readRemoteFile('/etc/fail2ban/action.d/ufw.conf', "robobee-${it}-test.test", 22222), "${args.test.name}_ufw_conf_expected.txt"
                    assertStringResource Fail2banServerTest, readRemoteFile('/etc/fail2ban/filter.d/sshd.conf', 'robobee-test.test'), "${args.test.name}_sshd_conf_expected.txt"
                }
            },
        ]
